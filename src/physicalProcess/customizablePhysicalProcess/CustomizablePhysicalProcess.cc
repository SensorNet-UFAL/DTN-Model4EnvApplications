...
void CustomizablePhysicalProcess::handleMessage(cMessage * msg)
{
	...
	switch (inputType) {
		...
		case DIRECT_LIST_ASSIGN:{ // Israel Vasconcelos (Jan, 2016) [FIXED!]

			if(valuesHash[nodeIndex].size() <= 1)
				break;
 
			char * toCharValuesList = strdup(valuesHash[nodeIndex].c_str());
			char * pch = strtok(toCharValuesList,"{,}-"); // Splitting by separators '{' ',' '}' 

			//trace() << valuesHash[nodeIndex];
	
			double returnValue = atof(pch); //TODO: Test double numbers at input (!!)

			receivedMsg->setValue(returnValue); // Encapsulating sample data
			send(receivedMsg, "toNode", nodeIndex); // IMPORTANT! Asserting data set to nodes HERE (!!)

			pch = strtok (NULL, "{,}-");

			string remainingSamples = "{";

			while (pch != NULL) { //Updating vector with the values 
				remainingSamples = remainingSamples + std::string(pch) + ",";
				pch = strtok (NULL, "{,}-");
			}

			remainingSamples[remainingSamples.size()-1] = '}'; //Adding the ending bracket

			valuesHash[nodeIndex] = remainingSamples;

			break;
		}
	}

	if(inputType != DIRECT_LIST_ASSIGN) { // Send reply back to the node who made the request
		...
		while (valuesTokenizer.hasMoreTokens()) {
			...
			if (isFirstToken) {
				...
				if ((posA != 0) || (posB == string::npos))
					opp_error("\nError!(A) Illegal parameter format \"directNodeValueAssignment\"\n");

				token = token.substr(1, token.size() - 1);

				if (token.size() < 1)
					opp_error("\nError!(B) Illegal parameter format \"directNodeValueAssignment\"\n");
				...
				if (tokenSize >= 3) {
					if ((posA == string::npos) || (posA == 0) || (posA == tokenSize - 1))
						opp_error("\nError! Illegal parameter format \"directNodeValueAssignment\"\n");
...
numSources);
	...
	// Israel Vasconcelos **************************************************************************************************
	// Oct, 2015
	// Changed valuesTable (double*) for valuesHash (string*), convert stoi when send the packet
	else if (inputType == DIRECT_LIST_ASSIGN) {
		cStringTokenizer valuesTokenizer(par("directNodeValueAssignment"), " ");
		int totalTokens = 0;
		int isFirstToken = 1;
		...
		while (valuesTokenizer.hasMoreTokens()) {
			...
			if (isFirstToken) {
				...
				defaultValue = atof(token.c_str());
				valuesHash = new string[numNodes];
				for (int i = 0; i < numNodes; i++)
					valuesHash[i] = token;
				...
				if (tokenSize >= 3) {
					...
					string strID = token.substr(0, posA); // From: Start, To: posA (':' splitted)
					int nodeId = atoi(strID.c_str());

					if (nodeId < 0 || nodeId > numNodes - 1)
						opp_error("\nError! Illegal parameter format \"directNodeValueAssignment\".\n");

					valuesHash[nodeId] = token.substr(posA + 1); // From: posA+1, To: End of str -> convert valuesHash[i] stoi before send
