...
/*
* Israel Vasconcelos. 
* Modified in Jun, 2016
* Changes: 
* - Adaptation of Multipath Rings to enable single hop communication with a mobile sink
* - Nodes can be discovered at any time during the simulation (not required preliminary configuration)
*/

#include "MyRouting.h"

Define_Module(MyRouting);

void MyRouting::startup()
{
	...
	isConnected = (isSink) ? true : true; // (ISR: Changed here - was: false:true)
...
void MyRouting::sendTopologySetupPacket()
{
	MyRoutingPacket *setupPkt =
	    new MyRoutingPacket("My routing setup packet", NETWORK_LAYER_PACKET);
	setupPkt->setMyRoutingPacketKind(MPRINGS_TOPOLOGY_SETUP_PACKET);
...
void MyRouting::sendControlMessage(MyRoutingControlDef kind)
{
	MyRoutingControlMessage *ctrlMsg =
	    new MyRoutingControlMessage("Multipath routing control message",NETWORK_CONTROL_MESSAGE);
	ctrlMsg->setMyRoutingControlMessageKind(kind);
	ctrlMsg->setLevel(currentLevel);
	ctrlMsg->setSinkID(currentSinkID);
	toApplicationLayer(ctrlMsg);
}

void MyRouting::timerFiredCallback(int index)
{
	...
	if (tmpLevel == NO_LEVEL) {
		...
		currentLevel = 1; // (ISR: Forcing all nodes to communicate in single-hop)
		currentSinkID = tmpSinkID;

		if (true) { // (CHANGED HERE (!!!) -> !isConnected for true) ######### PULO DO GATO AQUI !!!! ##############
			isConnected = true;
...
void MyRouting::processBufferedPacket()
{
...
void MyRouting::fromApplicationLayer(cPacket * pkt, const char *destination)
{
	...
	MyRoutingPacket *netPacket =
	    new MyRoutingPacket("My routing data packet", NETWORK_LAYER_PACKET);
	netPacket->setMyRoutingPacketKind(MPRINGS_DATA_PACKET);
...
{
	MyRoutingPacket *netPacket = dynamic_cast <MyRoutingPacket*>(pkt);
	if (!netPacket)
		return;

	switch (netPacket->getMyRoutingPacketKind()) {

		case MPRINGS_TOPOLOGY_SETUP_PACKET:{
			if (isSink)
				break;
			if (true) { // (ISR: CHANGED HERE!! - ALWAYS 'RECONFIGURE' NET TOPOLOGY)
				isScheduledNetSetupTimeout = true;
				setTimer(TOPOLOGY_SETUP_TIMEOUT, netSetupTimeout);
				tmpLevel = NO_LEVEL;
				tmpSinkID = NO_SINK;
		...
		case MPRINGS_DATA_PACKET:{
			...
			if (dst.compare(BROADCAST_NETWORK_ADDRESS) == 0 ||
				...
				toApplicationLayer(decapsulatePacket(pkt)); // (ISR: Sink always fetch every packet from MAC Layer)
