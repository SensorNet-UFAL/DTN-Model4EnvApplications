...
# nohup Rscript pairIndexes.R <samplingAlgorithm> <bufferSize> <sampleRate> <seed> &
# Example: nohup Rscript pairIndexes-SampleRate.R SampleCentral 1000 25 1 &
# Options: SampleCentral, DropRandom / 1000, 1500, 2000, 2500 / 10,25,50,75

evalType <- "varySampleRate" # Experiment under study

args <- commandArgs(TRUE)
args
samplingAlgorithm <- args[1] 
evalParameter <- as.double(args[2]) #bufferSize
sampleRate <- as.double(args[3]) 
seed <- as.double(args[4])
evalMetric <- args[5]
...
##############################################################################################################
	...
	remainingIndexes = read.tcsv(paste("RemainingIndexes/",seed,"-",evalType,"-",evalParameter,"-",sampleRate,"-",samplingAlgorithm,"-remainingIndexes.csv",sep=''))
	...
	# Israel, 2014
		...
		# Temporary variables
		r = seed
		n_sensores = sampleRate # Keep retrocompatibility with old names
		percentualReducao = evalParameter # = BufferSize / Keeping retrocompatibility old names
