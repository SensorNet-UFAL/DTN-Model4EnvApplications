for i in {1..20} # Repeat until succeed
do
...
if [ ! -e resultados-erro/$seed'_'$sampleRate'_'$bufferSize'_erroReconstrucao_'$algorithm'.dat' ] # Check if this seed is already evaluated
then
...
################ #################### #####################################################
	...
	then
		echo "$algorithm $sampleRate $bufferSize $seed Sampling-Breakpoint"
		break
	fi
done
done

sleep 2m # Wait 1 minutes for Castalia finish until run kriging
...
do 
	...
	do 
		...
		#---------------------------------------------------------------------
			...
			then 	
				...
				then
				    Rscript pairIndexes-SampleRate.R $algorithm $buffer $sampleRate  $seed $evalMetric
				else
				    nohup Rscript pairIndexes-SampleRate.R $algorithm $buffer $sampleRate  $seed $evalMetric &
				fi
			fi
		...
		#---------------------------------------------------------------------
			...
			if [ ! -e resultados-cobertura/$seed'_'$sampleRate'_'$bufferSize'_coverage_'$algorithm'.dat' ] # Check if this seed is already evaluated
			then 	
				if [ `expr $seed % $threads` -eq 0 ] # Parallelism control HERE 
				then
				    Rscript pairIndexes-SampleRate.R $algorithm $buffer $sampleRate $seed $evalMetric
	...
	if [ "$algorithm" = "DropRandom" ] # SampleRate varying is not applicable on DropRandom
	then
		echo "$algorithm $sampleRate $bufferSize $seed Kriging-Breakpoint"
		break
	fi
done
done
