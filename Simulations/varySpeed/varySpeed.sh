...
SN.node[*].Application.sinkSpeed = $speed # Same value as MobilityManager.speed (unable to request from .cc Application)
...
if [ ! -e resultados-erro/$seed'_50_'$speed'_erroReconstrucao_'$algorithm'.dat' ] # Check if this seed is already evaluated
then 	
...
do 
	...
	do
		...
		do
			...
			if [ ! -e resultados-cobertura/$seed'_50_'$speed'_coverage_'$algorithm'.dat' ] # Check if this seed is already evaluated
			then 	
				if [ `expr $seed % $threads` -eq 0 ] # Parallelism control HERE 
				then
				    Rscript pairIndexes-Speed.R $algorithm $speed $seed $evalMetric
				else
				    nohup Rscript pairIndexes-Speed.R $algorithm $speed $seed $evalMetric &
				fi
			fi
