...
for(i in r1) {
	...
	try(sc500[i] <- read.table(paste(i,"_50_2_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr500[i] <- read.table(paste(i,"_50_2_coverage_DropRandom.dat",sep=''))[[1]])

	try(sc1000[i] <- read.table(paste(i,"_50_3_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr1000[i] <- read.table(paste(i,"_50_3_coverage_DropRandom.dat",sep=''))[[1]])

	try(sc1500[i] <- read.table(paste(i,"_50_4_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr1500[i] <- read.table(paste(i,"_50_4_coverage_DropRandom.dat",sep=''))[[1]])

	try(sc2000[i] <- read.table(paste(i,"_50_5_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr2000[i] <- read.table(paste(i,"_50_5_coverage_DropRandom.dat",sep=''))[[1]])

	try(sc2500[i] <- read.table(paste(i,"_50_6_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr2500[i] <- read.table(paste(i,"_50_6_coverage_DropRandom.dat",sep=''))[[1]])
}
...
write.csv(comparisonMatrix, file="Speed-coverage (Comparison Matrix SC vs DR).csv")
...
setEPS()
postscript("graphic_varySpeedCoverage.eps")

plot(x, sc_tt[,2] ,ylim=y_lim, type='b', pch=4, col='black', ylab='Coverage (%)', xlab='Sink Speed (m/s).', xaxt="n")
axis(1, at=x, labels=c("2","3","4","5","6"))
