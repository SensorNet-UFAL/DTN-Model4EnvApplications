for i in {1..10} # Repeat until succeed
do
...
powerValues=('-25dBm' '-15dBm' '-10dBm' '-5dBm' '0dBm')
...
if [ ! -e resultados-erro/$seed'_50_'$power'_erroReconstrucao_'$algorithm'.dat' ] # Check if this seed is already evaluated
then 	
	...
	do
		...
		#---------------------------------------------------------------------
			...
			then 	
				...
				then
				    Rscript pairIndexes-Power.R $algorithm $power $seed $evalMetric
				else
				    nohup Rscript pairIndexes-Power.R $algorithm $power $seed $evalMetric &
				fi
			fi
		...
		#---------------------------------------------------------------------
			...
			if [ ! -e resultados-cobertura/$seed'_50_'$power'_coverage_'$algorithm'.dat' ] # Check if this seed is already evaluated
			then 	
				if [ `expr $seed % $threads` -eq 0 ] # Parallelism control HERE 
				then
				    Rscript pairIndexes-Power.R $algorithm $power $seed $evalMetric
