...
for(i in r1) {
	...
	try(sc500[i] <- read.table(paste(i,"_50_-25dBm_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr500[i] <- read.table(paste(i,"_50_-25dBm_coverage_DropRandom.dat",sep=''))[[1]])

	try(sc1000[i] <- read.table(paste(i,"_50_-15dBm_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr1000[i] <- read.table(paste(i,"_50_-15dBm_coverage_DropRandom.dat",sep=''))[[1]])

	try(sc1500[i] <- read.table(paste(i,"_50_-10dBm_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr1500[i] <- read.table(paste(i,"_50_-10dBm_coverage_DropRandom.dat",sep=''))[[1]])

	try(sc2000[i] <- read.table(paste(i,"_50_-5dBm_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr2000[i] <- read.table(paste(i,"_50_-5dBm_coverage_DropRandom.dat",sep=''))[[1]])

	try(sc2500[i] <- read.table(paste(i,"_50_0dBm_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr2500[i] <- read.table(paste(i,"_50_0dBm_coverage_DropRandom.dat",sep=''))[[1]])
}
...
write.csv(comparisonMatrix, file="Power-coverage (Comparison Matrix SC vs DR).csv")
...
setEPS()
postscript("graphic_varyPowerCoverage.eps")

plot(x, sc_tt[,2] ,ylim=y_lim, type='b', pch=4, col='black', ylab='Coverage (%)', xlab='Transmission power (dBm).', xaxt="n")
axis(1, at=x, labels=c("-25","-15","-10","-5","0"))
legend("topright", legend=c("Data-Aware Drop", "Drop Random Packet"), lty=1, col=c("black","red", "blue", "green", "gray"), bty="n", pch=c(4,5,6))
