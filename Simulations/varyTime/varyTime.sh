...
simTimes=(250 1000 2500 5000 10000)
...
if [ ! -e resultados-erro/$seed'_50_'$simTime'_erroReconstrucao_'$algorithm'.dat' ] # Check if this seed is already evaluated
then 	
...
do 
	...
	do
		...
		do
			...
			if [ ! -e resultados-cobertura/$seed'_50_'$simTime'_coverage_'$algorithm'.dat' ] # Check if this seed is already evaluated
			then 	
				if [ `expr $seed % $threads` -eq 0 ] # Parallelism control HERE 
				then
				    Rscript pairIndexes-Time.R $algorithm $simTime $seed $evalMetric
				else
				    nohup Rscript pairIndexes-Time.R $algorithm $simTime $seed $evalMetric &
				fi
			fi
