...
# Example: nohup Rscript pairIndexes-Event.R SampleCentral 1 &
# Options: SampleCentral, DropRandom / 0, 1

evalType <- "varyEvent" # Experiment under study

args <- commandArgs(TRUE)
args
samplingAlgorithm <- args[1] 
evalParameter <- as.double(args[2]) # Event: ON=1, OFF=0
...
if(length(args) != 3) {
	...
	if(evalParameter==0) 
		fieldFileName <- paste(seed,"-",numNodes,"-field.dat",sep='')
	if(evalParameter==1)
		fieldFileName <- paste(seed,"-",numNodes,"-",evalParameter,"-field.dat",sep='')

	field = read.table(paste(destinationFolder,fieldFileName,sep=''))
	...
	# Israel, 2014
		...
		r = seed
		n_sensores = numNodes
		percentualReducao = evalParameter # Keeping variables with old names to retrocompatibility
		...
		fileName2 <- paste("resultados-erro/",r,'_',n_sensores,'_',percentualReducao,'_coverage_',samplingAlgorithm,'.dat',sep='')
