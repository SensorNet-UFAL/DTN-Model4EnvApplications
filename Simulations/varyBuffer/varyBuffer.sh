	...
	do
		...
		#---------------------------------------------------------------------
			...
			then 	
				...
				then
				    Rscript pairIndexes-Buffer.R $algorithm $buffer $seed $evalMetric
				else
				    nohup Rscript pairIndexes-Buffer.R $algorithm $buffer $seed $evalMetric &
				fi
			fi
		...
		#---------------------------------------------------------------------
			...
			if [ ! -e resultados-cobertura/$seed'_50_'$bufferSize'_coverage_'$algorithm'.dat' ] # Check if this seed is already evaluated
			then 	
				if [ `expr $seed % $threads` -eq 0 ] # Parallelism control HERE 
				then
				    Rscript pairIndexes-Buffer.R $algorithm $buffer $seed $evalMetric
