...
for(i in r1) {
	...
	try(sc500[i] <- read.table(paste(i,"_50_500_erroReconstrucao_SampleCentral.dat",sep=''))[[1]])
	try(dr500[i] <- read.table(paste(i,"_50_500_erroReconstrucao_DropRandom.dat",sep=''))[[1]])
	...
	try(sc1500[i] <- read.table(paste(i,"_50_1500_erroReconstrucao_SampleCentral.dat",sep=''))[[1]])
	try(dr1500[i] <- read.table(paste(i,"_50_1500_erroReconstrucao_DropRandom.dat",sep=''))[[1]])

	try(sc2000[i] <- read.table(paste(i,"_50_2000_erroReconstrucao_SampleCentral.dat",sep=''))[[1]])
	try(dr2000[i] <- read.table(paste(i,"_50_2000_erroReconstrucao_DropRandom.dat",sep=''))[[1]])
}
...
write.csv(comparisonMatrix, file="Buffer-error (Comparison Matrix SC vs DR).csv")
...
setEPS()
postscript("graphic_varyBuffer.eps")
