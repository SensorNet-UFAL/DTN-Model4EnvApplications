...
for(i in r1) {
	...
	try(sc500[i] <- read.table(paste(i,"_20_x_erroReconstrucao_SampleCentral.dat",sep=''))[[1]])
	try(dr500[i] <- read.table(paste(i,"_20_x_erroReconstrucao_DropRandom.dat",sep=''))[[1]])

	try(sc1000[i] <- read.table(paste(i,"_35_x_erroReconstrucao_SampleCentral.dat",sep=''))[[1]])
	try(dr1000[i] <- read.table(paste(i,"_35_x_erroReconstrucao_DropRandom.dat",sep=''))[[1]])

	try(sc1500[i] <- read.table(paste(i,"_50_x_erroReconstrucao_SampleCentral.dat",sep=''))[[1]])
	try(dr1500[i] <- read.table(paste(i,"_50_x_erroReconstrucao_DropRandom.dat",sep=''))[[1]])

	try(sc2000[i] <- read.table(paste(i,"_100_x_erroReconstrucao_SampleCentral.dat",sep=''))[[1]])
	try(dr2000[i] <- read.table(paste(i,"_100_x_erroReconstrucao_DropRandom.dat",sep=''))[[1]])
}
...
write.csv(comparisonMatrix, file="Nodes-error (Comparison Matrix SC vs DR).csv")
...
setEPS()
postscript("graphic_varyNodes.eps")

plot(x, sc_tt[,2] ,ylim=y_lim, type='b', pch=4, col='black', ylab='Error (%)', xlab='Number of nodes.', xaxt="n")
