...
for(i in r1) {
	try(sc500[i] <- read.table(paste(i,"_20_x_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr500[i] <- read.table(paste(i,"_20_x_coverage_DropRandom.dat",sep=''))[[1]])
	try(idealsc500[i] <- read.table(paste(i,"_20_x_coverage_SampleCentral_Ideal.dat",sep=''))[[1]])
	try(idealdr500[i] <- read.table(paste(i,"_20_x_coverage_DropRandom_Ideal.dat",sep=''))[[1]])

	try(sc1000[i] <- read.table(paste(i,"_35_x_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr1000[i] <- read.table(paste(i,"_35_x_coverage_DropRandom.dat",sep=''))[[1]])
	try(idealsc1000[i] <- read.table(paste(i,"_35_x_coverage_SampleCentral_Ideal.dat",sep=''))[[1]])
	try(idealdr1000[i] <- read.table(paste(i,"_35_x_coverage_DropRandom_Ideal.dat",sep=''))[[1]])

	try(sc1500[i] <- read.table(paste(i,"_50_x_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr1500[i] <- read.table(paste(i,"_50_x_coverage_DropRandom.dat",sep=''))[[1]])
	try(idealsc1500[i] <- read.table(paste(i,"_50_x_coverage_SampleCentral_Ideal.dat",sep=''))[[1]])
	try(idealdr1500[i] <- read.table(paste(i,"_50_x_coverage_DropRandom_Ideal.dat",sep=''))[[1]])

	try(sc2000[i] <- read.table(paste(i,"_100_x_coverage_SampleCentral.dat",sep=''))[[1]])
	try(dr2000[i] <- read.table(paste(i,"_100_x_coverage_DropRandom.dat",sep=''))[[1]])
	try(idealsc2000[i] <- read.table(paste(i,"_100_x_coverage_SampleCentral_Ideal.dat",sep=''))[[1]])
	try(idealdr2000[i] <- read.table(paste(i,"_100_x_coverage_DropRandom_Ideal.dat",sep=''))[[1]])
...
write.csv(cbind(comparisonMatrix ,comparisonMatrixIdeal), file="Nodes-coverage (Comparison Matrix SC vs DR - Realistic and Ideal).csv")
...
setEPS()
postscript("graphic_varyNodesCoverageX4.eps")

plot(x, sc_tt[,2] ,ylim=y_lim, type='b', pch=4, col='black', ylab='Coverage (%)', xlab='Number of nodes.', xaxt="n")
